...
sub new {
    ...
    $this->{ignoreHosts} =
      join( '|', split( /\s*,\s*/, $this->{ignoreHosts} ) );
...
sub handleNrVisitors {
    ...
    my ($visitors) =
      $this->getVisitorsFromSessionStore( undef, $this->{ignoreUsers} );
...
sub handleNrGuests {
    ...
    my ( undef, $guests ) =
      $this->getVisitorsFromSessionStore( $Foswiki::cfg{DefaultUserWikiName} );
...
sub handleNrLastVisitors {
    ...
    return $this->{nrLastVisitors}{$theDays}
      if defined $this->{nrLastVisitors}{$theDays};

    my $visitors =
      $this->getVisitors( $theDays, undef, undef, $this->{ignoreUsers} );
...
sub handleVisitors {
    ...
    my $index  = 0;
    foreach
      my $visitor ( sort { $a->{wikiname} cmp $b->{wikiname} } @$visitors )
    {
    ...
    my $result =
        replaceVars($theHeader)
      . join( replaceVars($theSep), @result )
      . replaceVars($theFooter);
...
sub handleNewUsers {
    ...
    while ( $it->hasNext() ) {
        ...
        next
          unless Foswiki::Func::topicExists( $Foswiki::cfg{UsersWebName},
            $user );
        my ($date) =
          Foswiki::Func::getRevisionInfo( $Foswiki::cfg{UsersWebName},
            $user, 1 );
        push @users,
          {
            wikiname => $user,
        ...
        push @result,
          replaceVars(
            $line,
            {
                ...
                date =>
                  Foswiki::Func::formatTime( $user->{date}, '$day $mon $year' ),
            }
          );
...
sub handleLastVisitors {
    ...
    my $visitors =
      $this->getVisitors( $theDays, $theMax, undef, $this->{ignoreUsers} );
...
sub replaceVars {
    ...
    if ( defined $data ) {
        if ( defined $data->{wikiname} ) {
            $data->{username} =
              Foswiki::Func::wikiToUserName( $data->{wikiname} );
            $data->{wikiusername} =
              Foswiki::Func::userToWikiName( $data->{wikiname} );
        }
